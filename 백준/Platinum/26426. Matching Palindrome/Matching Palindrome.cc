...
void solve() {
  ...
  string u = s + t;
  vector<signed> z1 = z_algorithm(u);
  set<int> pre_palin;
  for (int i=2*n-1; i>=n; i--) {
    if (i-n == n - z1[i]) pre_palin.insert(i-n);
  }
  // string v = t + s;
  // vector<signed> z2 = z_algorithm(v);
  // set<int> suf_palin;
  // for (int i=2*n-1; i>=n; i--) {
  //   if (i-n == n - z2[i]) suf_palin.insert(i-n);
  // }
  // po(z1);
  // po(z2);
  // debug(pre_palin);
  ...
  set<int> ans;
  for (int i=n-1; i>=0; i--) {
    if (pre_palin.count(i)) {
      if (pre_palin.count(n-i)) ans.insert(n-i);
    }
  }
  if (ans.empty()) po(s);
  else {
    int len = *ans.begin();
    string v = s.substr(0,len);
