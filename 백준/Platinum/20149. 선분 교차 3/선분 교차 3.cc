...
void io_util() {
	...
	cout.precision(10); //cout의 소수점 범위를 9자리까지 늘린다. 4은 3.1415
}
...
pair<ld, ld> isec(pii a, pii b, pii c, pii d) {
    auto [ax, ay] = a;
    auto [bx, by] = b;
    auto [cx, cy] = c;
    auto [dx, dy] = d;

    int dx1 = bx - ax, dy1 = by - ay; 
    int dx2 = dx - cx, dy2 = dy - cy; 

    int D = dx1 * (-dy2) - (-dx2) * dy1;
    assert(D != 0);

    int det = (cx - ax) * (-dy2) - (cy - ay) * (-dx2);
    double t = (double)det / D;
...
void solve() {
    ...
    for (int i=0; i<4; i++) {
        ...
        if (dot(pq,orth(rs)) == 0) {
            auto pr = diff(p,r);
            auto qr = diff(q,r);
            if (dot(pr,qr) < 0 && dot(pr,orth(qr)) == 0) {
                po(1);
                return;
            }
        }
    ...
    for (int i=0; i<4; i++) {
        ...
        if (make_pair(p,q) == make_pair(r,s)) {
            po(1);
            return;
        }
        
        if (make_pair(p,q) == make_pair(s,r)) {
            po(1);
            return;
        }
    }
    ...
    auto [x,y] = isec(p,q,r,s);
    auto [px,py] = p;
    auto [qx,qy] = q;
    auto [rx,ry] = r;
    auto [sx,sy] = s;

    auto check = [&] (ld l, ld r, ld x) {
        ld eps = 1e-9;
        if (l > r) swap(l,r);
