...
void solve() {
    ...
    auto f = [&] (int X) {
        ...
        int ret = 0;
        for (int i=1; i<=X; i*=10) ret++;

        int sz = 9*n;
        vi primes = {2,3,5,7};
        for (int tg=2; tg<=sz; tg++) {
            ...
            // length, sum, product remainder, bound flag, leading zero flag
            // Vec<5,int> dp(n+1,tg+2,tg,2,2);
            // dp[0][0][1][0][0] = 1;
            for (int i=0; i<=n; i++) {
            ...
            for (int i=0; i<n; i++) {
                for (int sm=0; sm<=tg; sm++) {
                    for (int r=0; r<tg; r++) {
                        for (int bd=0; bd<2; bd++) {
                            for (int lz=0; lz<2; lz++) {
                                if (dp[i][sm][r][bd][lz] == 0) continue;
                                for (int d=(lz?1:0); d<=(bd?9:a[i]); d++) {
                                    int nsm = min(sm + d,tg+1);
                                    int nbd = bd | (d < a[i]);
                                    int nlz = lz | (d != 0);
                                    int nr = (nlz) ? ((r * d) % tg) : r;
                                    dp[i+1][nsm][nr][nbd][nlz] += dp[i][sm][r][bd][lz];
                                }
    ...
    auto g = [&] (int X) {
        ...
        vi a(n);
        for (int i=0; i<n; i++) a[i] = s[i] - '0';
        // count of numbers having digit 0 that is less than X
        // bd
        // leading zeros
        // zero exists
        Vec<4,int> dp(n+1,2,2,2);
        dp[0][0][0][0] = 1;
        for (int i=0; i<n; i++) {
            for (int bd=0; bd<2; bd++) {
                for (int lz=0; lz<2; lz++) {
                    for (int z=0; z<2; z++) {
                        for (int d=0; d<=(bd?9:a[i]); d++) {
                            int nbd = bd | (d < a[i]);
                            int nlz = lz | (d != 0);
                            int nz = z | (lz && (d==0));
                            dp[i+1][nbd][nlz][nz] += dp[i][bd][lz][z];
                        }
                    }
                }
            }
