...
void solve() {
    ...
    vi comp(a.begin(), a.end());
    for (auto &x : qr) comp.push_back(x);
    ...
    function<int(int,int)> f = [&] (int i, int j) {
        ...
        int ret = inf;
        if (2*i+2 < n) {
            int cur_cost = (b[i]==j)?0ll:c[i];
            int u = 2*i+1;
            int v = 2*i+2;
            chmin(ret,cur_cost+f(u,j));
            chmin(ret,cur_cost+f(v,j));
            chmin(ret,f(u,j)+f(v,j));
            if (j == 1) {
                if (b[i] == j) {
                    chmin(ret, f(u,0)+f(v,2));
                    chmin(ret, f(u,2)+f(v,0));
                }
                if (b[i] > j) {
                    chmin(ret, f(u,0)+f(v,1));
                    chmin(ret, f(u,1)+f(v,0));
                }
                if (b[i] < j) {
                    chmin(ret, f(u,2)+f(v,1));
                    chmin(ret, f(u,1)+f(v,2));
                }
            }
        }
        else {
            chmin(ret, (b[i]==j)?0ll:c[i]);
        }
        return dp[i][j] = ret;
    };
    ...
    auto g = [&] (int i, int j) {
        ...
        if (2*i+2 < n) {
            ...
            int u = 2*i+1;
            int v = 2*i+2;
            chmin(ret,cur_cost+dp[u][j]);
            chmin(ret,cur_cost+dp[v][j]);
            chmin(ret,dp[u][j]+dp[v][j]);
            if (j == 1) {
                if (b[i] == j) {
                    chmin(ret, dp[u][0] + dp[v][2]);
                    chmin(ret, dp[u][2] + dp[v][0]);
                }
                if (b[i] > j) {
                    chmin(ret, dp[u][0] + dp[v][1]);
                    chmin(ret, dp[u][1] + dp[v][0]); 
                }
                if (b[i] < j) {
                    chmin(ret, dp[u][2] + dp[v][1]); 
                    chmin(ret, dp[u][1] + dp[v][2]);
                }
            }
        }
        else {
