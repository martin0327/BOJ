...
void solve() {
    for (int k=1; k<=10; k++) {
        int max_rem = k*(k+1)/2;
        VIS[k] = DP[k] = vvi(1<<k, vi(max_rem));
        auto &dp = DP[k];
        auto &vis = VIS[k];

        function<int(int,int)> f = [&] (int mask, int rem) {
            int ret = 0;
            if (rem <= 0) return dp[mask][0] = ret;
            if (vis[mask][rem]) return ret = dp[mask][rem];
            vis[mask][rem] = 1;
