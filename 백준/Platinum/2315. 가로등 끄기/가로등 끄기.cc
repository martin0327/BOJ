...
void solve() {
    ...
    function<int(int,int,int)> f = [&] (int l, int r, int z) {
        if (dp[l][r][z] != -1) return dp[l][r][z];
        if (l == r) {
            if (l == m) return dp[l][r][z] = 0;
            else return inf;
        }
        int ret = inf;
        if (z == 0) {
            int x = f(l+1,r,0);
            if (x < inf) x += abs(a[l]-a[l+1]) * (tot - (pre[r+1]-pre[l+1]));//query(l+1,r));
            int y = f(l+1,r,1);
            if (y < inf) y += abs(a[l]-a[r]) * (tot - (pre[r+1]-pre[l+1])); //)query(l+1,r));
            chmin(ret,x);
            chmin(ret,y);
        }
        else {
            int x = f(l,r-1,0);
            if (x < inf) x += abs(a[l]-a[r]) * (tot - (pre[r]-pre[l])); //)query(l,r-1));
            int y = f(l,r-1,1);
            if (y < inf) y += abs(a[r-1]-a[r]) * (tot - (pre[r]-pre[l])); //query(l,r-1));
            chmin(ret,x);
            chmin(ret,y);
        }
        return dp[l][r][z] = ret;
