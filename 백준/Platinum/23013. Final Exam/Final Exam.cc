...
void solve() {
    ...
    for (auto x : b) {
        ...
        if (l1 <= x && x <= l2) {
            ans.push_back(x);
            s.erase(prev(it));
            if (l1 <= x-1) s.insert({l1,x-1});
            if (x+1 <= l2) s.insert({x+1,l2});
        }
        else if (r1 <= x && x <= r2) {
            ans.push_back(x);
            s.erase(it);
            if (r1 <= x-1) s.insert({r1,x-1});
            if (x+1 <= r2) s.insert({x+1,r2});
        }
        else {
            int dl = abs(x-l2), dr = abs(x-r1);
            if (dl <= dr) {
                ans.push_back(l2);
                s.erase(prev(it));
                if (l1 <= l2-1) s.insert({l1,l2-1});
            }
            else {
                ans.push_back(r1);
                s.erase(it);
                if (r1+1 <= r2) s.insert({r1+1,r2});
            }
        }
    }
    po(ans);
