...
void solve() {
    ...
    asort(a);
    n = a.size();
    int p = lower_bound(a.begin(), a.end(), 0ll) - a.begin();
    Vec<3,map<int,int>> dp(n,n,2);
    dp[p][p][0][0] = 0;
    dp[p][p][1][0] = 0;
    // dp[l][r] = [l,r] 을 cover했을 때 (cur_dist, tot_dist)
    function<map<int,int>(int,int,int)> f = [&] (int l, int r, int j) {
        if (dp[l][r][j].size()) {
            return dp[l][r][j];
        }
        map<int,int> ret;
        ...
        for (int pj=0; pj<2; pj++) {
            for (auto [pl,pr] : t) {
                auto nb = f(pl,pr,pj);
                auto g = [&] (int d) {
                    for (auto [cd,td] : nb) { // cur_dist, tot_dist
                        int ncd = cd + d;
                        chmin(ncd,m);
                        int ntd = td + ncd;
                        ...
                        auto it = ret.lower_bound(ncd);
                        if (it == ret.begin()) {
                            if (ncd < it->first) {
                                ret[ncd] = ntd;
                                if (it->second >= ntd) ret.erase(it);
                            }
                            else {
                                assert(ncd == it->first);
                                chmin(it->second, ntd);
                ...
                if (l == pl) { 
                    ...
                    // r == pr i.e. l+1 -> l
                    // j must be 0
                    int d = pj ? abs(a[l] - a[r]) : abs(a[l] - a[l+1]);
                    g(d);
                }
            }
        }
        
        // dp[l][r][0] <- dp[l+1][r][0], dp[l+1][r][1],dp[l][r-1][0], dp[l][r-1][1]

        return dp[l][r][j] = ret;
    };
    int res = 2e18;
    for (auto &[k,v] : f(0,n-1,0)) chmin(res, v);
    for (auto &[k,v] : f(0,n-1,1)) chmin(res, v);
    int ans = nn * m - res;
    po(ans);
}