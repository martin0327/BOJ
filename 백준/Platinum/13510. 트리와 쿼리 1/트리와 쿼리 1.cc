...
void solve() {
    ...
    auto euler_idx = [&] (int u) {
        return ett.ETT[u].first;
    };

    for (int i=0; i<n; i++) {
        seg.set(euler_idx(i), a[i]);
    }
    ...
    while (q--) {
        ...
        if (t==1) {
            ...
            int z = ett.lca(u,v);
            int ans = 0;
            int z_idx = euler_idx(z);

            for (auto [l,r] : ett.path(u,z)) {
                if (l == z_idx) l++;
                chmax(ans, seg.prod(l,r));
            }
            for (auto [l,r] : ett.path(v,z)) {
                if (l == z_idx) l++;
                chmax(ans, seg.prod(l,r));
            }
            po(ans);
