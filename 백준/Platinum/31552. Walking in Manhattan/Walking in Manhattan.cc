...
void solve() {
    ...
    for (auto x : H) {
        if (x&1) H1.push_back(x);
        else H2.push_back(x);
    }

    const int m = 19;
    vvi vsp(m, vi(vsz, inf));
    vvi hsp(m, vi(hsz, inf));

    auto g = [&] (vvi &vsp, vi &V, vi &V1, vi &V2) {
        int vsz = V.size();
        for (int i=0; i<vsz; i++) {
            int x = V[i];
            auto &V3 = (x&1) ? V2 : V1;
            auto it = lower_bound(V3.begin(), V3.end(), x);
            if (it != V3.end()) {
                int nx = *it;
                auto it2 = lower_bound(V.begin(), V.end(), nx);
                if (it2 != V.end()) {
                    vsp[0][i] = it2 - V.begin();
                }
            }
    ...
    auto f = [&] (int x, int y, int d) {
        ...
        bool b1 = (itx != V.end()) && (*itx == x);
        bool b2 = (ity != H.end()) && (*ity == y);
        ti3 ret;
        if (b1 && b2) ret = {x,y,d};
        else if (b1) {
            if (ity == H.end()) ret = {x,y+d,0};
            else {
                int diff = min(d, *ity - y);
                d -= diff;
                y += diff;
                ret = {x,y,d};
            }
            ...
            if (itx == V.end()) ret = {x+d,y,0};
            else {
                int diff = min(d, *itx - x);
                d -= diff;
                x += diff;
                ret = {x,y,d};
            }
    ...
    while (q--) {
        ...
        if ((d0-d)&1) { // east first
            ...
            int nid = (hid < hsz) ? hsp[0][hid] : inf;
            if (nid < hsz) {
                int diff = min(d, H[nid] - H[hid]);
                y += diff;
                d -= diff;
                x += d;
            }
