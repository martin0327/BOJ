...
void solve() {
    ...
    // map<int,int> mp;
    // for (int i=0; i<sz; i++) {
    //     if (!mp.count(c[i])) mp[c[i]] = i;
    // }
    vi blk;
    for (int i=0; i<sz; i++) {
        if (i == c[i]) blk.push_back(i);
    }
    // auto uc = get_unique(c);
    // debug(c);
    // debug(mp);
    vi dp(sz);
    for (auto [u,v,i] : qr) {
        ...
        if (max(l1,l2) > min(r1,r2)) {
            int l = min(r1,r2);
            int r = max(l1,l2);
            auto it = lower_bound(blk.begin(), blk.end(), l);
            if (it != blk.end() && *it < r) {
                po(-1);
            }
            else {
                int lo = 1, hi = sz, ans = -1;
                while (lo <= hi) {
                    int mid = (lo+hi)/2;
                    int cur = l;
                    for (int i=0; i<k; i++) {
                ...
                // auto idx = [&] (int x) {
                //     auto it = lower_bound(uc.begin(), uc.end(), x);
                //     return it - uc.begin();
                // };
                // int ans = idx(c[r]) - idx(c[l]) + 1;
                // // int diff = idx(r) - idx(l);
                // // debug(l,r,c[l],c[r],ans);
                // // if (r > 0 && c[r] != c[r-1]) ans--;
                // // po(ans);
