...
void solve() {
    ...
    for (int d=0; d<n; d++) {
        for (int l=0; l+d<n; l++) {
            ...
            if (d==0) {
                ...
                dp1[l][r] = dp1[l+1][r] + dp1[l][r-1] - dp1[l+1][r-1];
                if (a[l][r]) dp1[l][r] += dp1[l+1][r-1] + 1;
            }
        }
    }
    vvm dp2(n, vm(n));
    for (int d=n-1; d>=0; d--) {
        for (int l=0; l+d<n; l++) {
            int r = l+d;
            if (l-1 >= 0 && r+1 < n) {
                dp2[l][r] = dp2[l-1][r] + dp2[l][r+1] - dp2[l-1][r+1];
                if (a[l-1][r+1]) dp2[l][r] += dp2[l-1][r+1] + 1; 
            }
        }
    }
    vm res(n);
        ...
        for (int j=i; j<n; j++) {
            if (a[i][j]) {
                if (i==j) {
                    ...
                    mint x = (dp1[i+1][j-1]+1)*(dp2[i][j]+1);
                    res[i] += x;
                    res[j] += x;
                }
            }
