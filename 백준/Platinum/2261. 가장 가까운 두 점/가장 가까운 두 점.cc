...
void solve() {
    ...
    function<int(int,int)> f = [&] (int l, int r) {
        ...
        for (int i=l,j=m+1,idx=l;idx<=r;) {
            if (i>m) aa[idx++] = a[j++];
            else if (j>r) aa[idx++] = a[i++];
            else {
                auto [xi,yi] = a[i];
                auto [xj,yj] = a[j];
                if (yi <= yj) aa[idx++] = a[i++];
        ...
        for (int i=l; i<=r; i++) {
            auto [x,y] = a[i];
            if (abs(xm-x) < h) b.push_back({x,y});
        }
        
        int sz = b.size();
        // for (int i=1; i<sz; i++) {
        //     assert(b[i].second >= b[i-1].second);
        // }
