...
void solve() {
    ...
    int sz = 5e4+1;
    fenwick_tree<int> fen1(sz), fen2(sz);

    int ans = 0;
    for (auto [x,i] : a) {
        int cntl = fen1.sum(0,i);
        int cuml = fen2.sum(0,i);

        int cntr = fen1.sum(i+1,sz);
        int cumr = fen2.sum(i+1,sz);

        auto f = [&] (int cnt, int cum) {
            return abs(cum - i*cnt);
        };
