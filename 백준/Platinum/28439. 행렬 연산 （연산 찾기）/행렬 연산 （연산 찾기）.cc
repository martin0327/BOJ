...
void solve() {
    ...
    for (auto &x : a) ri(x);
    map<int,int> mp;
    for (int i=0; i<n; i++) mp[a[0][0]-a[i][0]]++;
    for (int j=0; j<m; j++) mp[a[0][j]]++;
    vp b;
    for (auto [k,v] : mp) b.push_back({v,k});
    int x = (*max_element(b.begin(), b.end())).second;
    vi r(n),c(m);
    for (int i=0; i<n; i++) {
        r[i] = a[i][0]-a[0][0]+x;
    }
    ...
    for (int i=0; i<n; i++) {
        if (r[i]) {
            for (int j=0; j<m; j++) d[i][j] += r[i];
            ans.emplace_back(1,i+1,r[i]);
        }
    }
    for (int j=0; j<m; j++) {
        if (c[j]) {
            for (int i=0; i<n; i++) d[i][j] += c[j];
            ans.emplace_back(2,j+1,c[j]);
        }
    }
    if (a==d) {
        po(ans.size());
        for (auto [t,i,v] : ans) po(t,i,v);
    }
    else po(-1);
}