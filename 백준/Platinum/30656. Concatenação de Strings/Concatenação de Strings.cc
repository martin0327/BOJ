...
vector<i32> prefix_function(const string &s) {
    i32 n = (i32)s.size();
    vector<i32> pi(n);
    for (i32 i = 1; i < n; ++i) {
        i32 j = pi[i-1];
...
i32 minimal_period(const string &s) {
    i32 n = (i32)s.size();
    if (n == 1) return 1;
...
signed main() {
    ...
    while (cin >> r >> s) {
        ...
        i32 p1 = minimal_period(a);
        i32 p2 = minimal_period(b);

        if (a.compare(0, p1, b, 0, p2) != 0) {
            cout << "NAO\n";
            continue;
        }
