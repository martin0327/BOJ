...
void solve() {
    ...
    for (int i=0; i+k-1<n; i++) {
        for (int mask=0; mask<(1<<k); mask++) {
            if (p6[mask]) continue;
            if (p5[mask/2]) continue;
            if (p5[mask&31]) continue;
            bool ok = true;
            for (int j=0; j<k; j++) {
                if (s[i+j] == '?') continue;
                if ((s[i+j] - '0') != (mask>>j&1)) ok = false;
            }
            if (!ok) continue;
            if (i==0) a[i][mask] = 1;
            else {
                int pmask = mask & 31;
                pmask *= 2;
                if (a[i-1][pmask] || a[i-1][pmask+1]) a[i][mask] = 1;
            }
        }
    }
    if (sum(a.back()) > 0) po("POSSIBLE");
    else po("IMPOSSIBLE");
