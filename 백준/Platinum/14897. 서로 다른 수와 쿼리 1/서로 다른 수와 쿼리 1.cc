...
void solve() {
    ...
    vector<i32> a(n); ri(a);
    auto comp = get_unique(a);
    for (i32 i=0; i<n; i++) {
        auto it = lower_bound(comp.begin(), comp.end(),a[i]);
        a[i] = it - comp.begin();
    }
    i32 sz = comp.size();
    ...
    // map<int,vti3> mo;
    i32 m = sqrt(n);
    vector<vti3> mo((n/m) + 3);
    for (i32 i=0; i<q; i++) {
        i32 l,r; 
        ri(l,r); 
        l--; r--;
        assert(l/m < (int) mo.size());
        mo[l/m].push_back({r,l,i});
    }
    vector<i32> ans(q);
    ...
    // for (auto &[qt,rli] : mo) {
    for (auto &rli : mo) {
        if (i%2==0) sort(rli.begin(), rli.end());
        else sort(rli.rbegin(), rli.rend());
