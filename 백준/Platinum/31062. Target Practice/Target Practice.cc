...
void solve(int n, int m, vi a, string s); 
void solve2(int n, int m, vi a, string s);
signed main() {
    io_util();
...
void inspect(vi &a) {
    int m = a.size();
    vi b;
    for (int i=0; i<m; i++) if (a[i] > 0) b.push_back(i);
    debug(b);
}
...
void solve(int n, int m, vi a, string s) {
    ...
    for (int i=0; i<m; i++) {
        if (s[i] == 'L') { 
            { // F -> +1
                int temp = cnt[2] + cnt[3];
                if (fire[2][cur] == 0 && fire[3][cur] == 0 && b[cur]) temp++;
                // debug(temp,cnt[2],cnt[3]);
                chmax(ans, temp);
            }
            { // R -> +2
                int temp = cnt[2] + cnt[4];
            ...
            { // F -> -1
                ...
                if (fire[2][cur] == 0 && fire[1][cur] == 0 && b[cur]) temp++;
                if (i==6) {
                    debug(cnt[2], cnt[1]);
                }
                chmax(ans, temp);
        ...
        else {
            {
                // L -> -1
                int temp = cnt[2] + cnt[1];
                if (fire[1][cur-1] == 1 && b[cur-1]) temp--;
            ...
            }
            {
                // R -> +1
                int temp = cnt[2] + cnt[3];
                if (fire[3][cur+1] == 1 && b[cur+1]) temp--;
                
                chmax(ans, temp);
            }
            fire[2][cur]++;
            if (fire[2][cur] == 1 && b[cur]) {
                cnt[2]++;
                for (int j=0; j<5; j++) {
                    if (j==2) continue;
                    if (fire[j][cur] > 0) {
        ...
        if (s[i] == 'L') {
            {
                ...
                for (int j=i+1; j<m; j++) {
                    ...
                    else if (sa.count(temp)) tt.insert(temp);
                }
                chmax(ans, (int) tt.size());
            }
            {
                // L -> F
                int temp = cur;
                set<int> tt = ss;
                if (sa.count(temp)) tt.insert(temp);
                for (int j=i+1; j<m; j++) {
                    if (s[j] == 'L') temp--;
                    else if (s[j] == 'R') temp++;
                    else if (sa.count(temp)) tt.insert(temp);
            ...
            if (sa.count(cur)) ss.insert(cur);
        }
    }
    chmax(ans, (int) ss.size());
    po(ans);
