...
void solve() {
    ...
    for (int i=0; i<m; i++) {
        ...
        int v_in = v;
        int v_out = v+n;
        g.add_edge(u_out,v_in,inf);
        g.add_edge(v_out,u_in,inf);
        s.emplace(u_out,v_in);
        s.emplace(v_out,u_in);
    }
    for (int i=0; i<n; i++) {
        int u = i;
        int u_in = u;
        int u_out = u+n;
        g.add_edge(u_in,u_out,a[i]);
        s.emplace(u_in,u_out);
    }

    tg += n;
    g.flow(src,tg);
    // int temp = g.flow(src,tg);
    // debug(temp);
    auto mc = g.min_cut(src);
    // {
    //     vi temp(mc.begin(), mc.end());
    //     debug(temp);
    // }
    vi ans;
    for (auto [u,v] : s) {
