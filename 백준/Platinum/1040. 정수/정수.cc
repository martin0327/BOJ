...
void solve() {
    ...
    for (int mask=0; mask<(1<<m); mask++) {
        ...
        vi d;
        for (int i=0; i<m; i++) {
            if (mask>>i&1) d.push_back(i);
        }

        auto f = [&] (vi &d) {
            Vec<3,i128> dp(n+1,1<<k,2,inf);
            dp[0][0][0] = 0;
            for (int i=0; i<n; i++) {
                for (int mask=0; mask<(1<<k); mask++) {
                    for (int j=0; j<2; j++) {
                        ...
                        for (int l=0; l<k; l++) {
                            if (j == 0 && d[l] < a[i]) continue;
                            int nmask = mask | (1<<l);
                            int nj = j | (d[l] > a[i]);
                            i128 v = dp[i][mask][j] * 10 + d[l];
                            chmin(dp[i+1][nmask][nj], v);
                        }
                    }
                }
            }
            int full = (1<<k) - 1;
            return min(dp[n][full]);
        };

        auto g = [&] (vi &d) {
            int len = max(n+1,k);
            Vec<3,i128> dp(len+1,1<<k,2,inf);
            dp[0][0][0] = 0;
            for (int i=0; i<len; i++) {
                for (int mask=0; mask<(1<<k); mask++) {
                    for (int j=0; j<2; j++) {
