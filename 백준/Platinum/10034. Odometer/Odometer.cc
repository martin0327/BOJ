...
void solve() {
    ...
    auto f = [&] (int t) {
        ...
        // length, leading zero cnt, bound, count of target digit
        int ret = 0;
        for (int tg=0; tg<10; tg++) {
            Vec<4,int> dp(n+1,n+1,2,n+1);
            dp[0][0][0][0] = 1;
            for (int i=0; i<n; i++) {
                for (int lz=0; lz<=i; lz++) {
                    for (int bd=0; bd<2; bd++) {
                        for (int ct=0; ct<=i; ct++) {
                            if (dp[i][lz][bd][ct] == 0) continue;
                            for (int d=0; d<=(bd?9:a[i]); d++) {
                                int nlz = lz + (d == 0 && lz == i);
                                int nbd = bd | (d < a[i]);
                                int nct = ct + (tg == d);
                                if (tg==0) nct = ct + (lz != i && tg == d); 
                                dp[i+1][nlz][nbd][nct] += dp[i][lz][bd][ct];
                            }
                        }
                    }
                }
            ...
            for (int lz=0; lz<n; lz++) {
                for (int bd=0; bd<2; bd++) {
                    for (int ct=0; ct<=n; ct++) {
                        if (n-lz-ct <= ct) ret += dp[n][lz][bd][ct];
                    }
                }
            }
        }
