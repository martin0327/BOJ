...
static const i128 MOD = (1ULL << 61) - 1;

inline i128 modmul(i128 a, i128 b) {
    i128 ret = a * b;
    ret %= MOD;
    return ret;
}

inline i128 modadd(i128 a, i128 b) {
    i128 ret = a + b;
    ret %= MOD;
    return ret;
}
inline i128 modsub(i128 a, i128 b) {
    i128 ret = a - b;
    ret %= MOD;
    ret += MOD;
    ret %= MOD;
...
i128 modpow(i128 a, i128 e) {
    i128 res = 1;
    while (e) {
        if (e & 1) res = modmul(res, a);
        a = modmul(a, a);
        e >>= 1;
    }
    return res;
...
struct rabin_karp {
    int n;
    i128 p;
    vector<i128> p_pow, p_inv, h;

    rabin_karp(const string &s, uint64_t p = 7817)
      : n((int)s.size()), p(p),
        p_pow(n+1), p_inv(n+1), h(n+1)
    {
        p_pow[0] = 1;
        for (int i = 1; i <= n; i++)
            p_pow[i] = modmul(p_pow[i-1], p);

        p_inv[n] = modpow(p_pow[n], MOD - 2);

        for (int i = n; i > 0; i--)
            p_inv[i-1] = modmul(p_inv[i], p);

        for (int i = 0; i < n; i++) {
            int x = ('a' <= s[i] && s[i] <= 'z')
                  ? (s[i] - 'a' + 1)
                  : (s[i] - 'A' + 27);
            h[i+1] = modadd(h[i], modmul(x, p_pow[i]));
        }
    }

    i128 query(int l, int r) const {
        i128 diff = modsub(h[r+1], h[l]);
        return modmul(diff, p_inv[l]);
    }
};
...
void solve() {
    ...
    while (m--) {
        ...
        auto rk = rabin_karp(t);
        int sz = t.size();
        mp[sz][rk.query(0,sz-1)]++;
        // debug(t,sz);
    }
    vi keys;
    for (auto &[k,v] : mp) {
    ...
    for (int i=0; i<n; i++) {
        for (auto k : keys) {
            if (i+k-1 < n) {
                auto h = rk.query(i,i+k-1);
                if (mp[k].count(h)) {
                    adj[i].push_back({i+k, mp[k][h]});
                }
            }
        }
    }
