...
void solve() {
    signed n,m; ri(n,m);
    vector<vector<signed>> a(n, vector<signed>(m));
    signed mx = 0;
    ...
    signed ans = 0;
    dsu d(n*m);
    for (signed h=0; h<=mx; h++) {
        d.init();
        for (signed i=0; i<n; i++) {
            for (signed j=0; j<m; j++) {
                // b[i][j] = (a[i][j] > h);
                vg[i*m+j] = (i == 0 || i == n-1 || j == 0 || j == m-1);//verge(i,j);
            }
        }
        ...
        for (signed i=0; i<n; i++) {
            for (signed j=0; j<m; j++) {
                ...
                for (signed l=0; l<2; l++) {
                    ...
                    d.merge(u,v);
                    if (vg[u] || vg[v]) vg[d.leader(u)] = 1;
                }
            }
        }
        for (signed l=0; l<n*m; l++) {
            if (l == d.leader(l)) {
                signed r = l/m, c = l%m;
                if (!vg[l] && !(a[r][c] > h)) {
                    ans += d.size(l);
