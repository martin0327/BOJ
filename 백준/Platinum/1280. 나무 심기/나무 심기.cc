...
void solve() {
    ...
    vi a(n); ri(a);
    const int mx = 2e5+1;
    fenwick_tree<int> fen1(mx), fen2(mx);
    mint ans = 1;
    fen1.add(a.front(),1);
    fen2.add(a.front(),a.front());
    a.erase(a.begin());
    for (auto x : a) {
        int cntL = fen1.sum(0,x);
        int valL = fen2.sum(0,x);
        int cntR = fen1.sum(x+1,mx);
        int valR = fen2.sum(x+1,mx);
        mint val = x*cntL - valL;
        val += valR - x*cntR;
        ans *= val;
