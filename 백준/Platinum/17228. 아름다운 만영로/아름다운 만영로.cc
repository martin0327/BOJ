...
struct rabin_karp {
    ...
    rabin_karp(string s, i128 p, i128 m) {
        this->n = s.size();
        this->p = p;
        this->m = m;
        p_pow = p_inv = vector<i128> (n+1,1);
        for (int i=1; i<=n; i++) p_pow[i] = (p_pow[i-1]*p)%m;
        p_inv[n] = pm(p_pow[n],m-2,m);
        for (int i=n-1; i>0; i--) p_inv[i] = (p_inv[i+1]*p)%m;
        h = vector<i128> (n+1);
        for (int i=0; i<n; i++) {
            i128 x = ('a'<=s[i] && s[i]<='z') ? (s[i]-'a'+1) : (s[i]-'A'+27);
            h[i+1] = (h[i]+(x*p_pow[i]))%m;
        }
    }
...
const int mx_sz = 5e5+5;
i128 p_pow[mx_sz];
i128 p_inv[mx_sz];
const i128 p = 31;
const i128 m = 914217179575780481;
...
void solve() {
    ...
    segtree<S,op,e> seg(mx_sz);

    string s; ri(s);
    int sz = s.size();
    for (int i=0; i<sz; i++) {
        i128 w = (p_pow[i+1] * (s[i]-'a'+1)) % m;
        seg.set(i+1,w);
    }
    auto tg = seg.all_prod();
    // tg = (tg+m) % m;
    // debug((int) tg);

    int ans = 0;
    function<void(int,int,int)> g = [&] (int u, int p, int i) {
        i128 w = (p_pow[i] * (a[u])) % m;
        seg.set(i,w);
        if (i >= sz) {
            auto s = seg.prod(i-(sz-1),i+1);
            s *= p_inv[i-sz];
            s %= m;
            // debug(u,(int) s);
            if (s == tg) ans++;
