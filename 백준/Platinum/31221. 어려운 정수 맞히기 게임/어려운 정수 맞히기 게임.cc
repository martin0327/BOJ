...
void solve() {
    ...
    bool done = false;
    int mxx = 1e18;
    auto f = [&] (int z, int lim) {
        int lo = 0, hi = lim, ret = -1;
        while (lo <= hi) {
            int mid = (lo+hi)/2;
            char q = query(z,mid);
            if (q == '0') {
    ...
    for (int i=1; i<sz; i++) {
        if (done) {
            int ans = 0;
            for (auto x : a) ans += sq(x);
            answer(ans);
            debug(tot);
            return;
        }
        int in = 0;
        for (int j=0; j<i; j++) in += sq(a[j]);
        if (i==1) a[i] = f(in, isqrt(mxx));
        else a[i] = f(in,isqrt(2*a[i-1]));
    }
}