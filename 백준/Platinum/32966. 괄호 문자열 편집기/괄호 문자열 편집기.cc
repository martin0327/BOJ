...
void solve() {
    ...
    for (int i=0; i<n; i++) {
        ...
        if (c == '(') {
            ...
            if (L.size()) {
                if (pre.back() < 0) neg_cnt--;
                R.push_back(pre.back() - delta);
                if (suf.empty()) suf.push_back(R.back());
                else suf.push_back(min(suf.back(), R.back()));
                L.pop_back();
                pre.pop_back();
            }
        }
            ...
            if (R.size()) {
                L.push_back(R.back() + delta);
                pre.push_back(pre.back() + L.back());
                R.pop_back();
                suf.pop_back();
                if (pre.back() < 0) neg_cnt++;
                if (R.empty()) delta = 0;
