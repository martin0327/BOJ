...
void solve() {
    ...
    for (int l=1; l<=n; l++) {
        for (int j=0; j<l; j++) {
            if (j == 0) tr[l][j] = a[l];
            else tr[l][j] = pre[l-1] - pre[j-1] + a[l];
        }
    }
    ...
    for (int l=1; l<=n; l++) {
        int x = a[l];
        dp2.assign(n+1,bt(0));
        // for (auto &v : dp2) v.assign(n+1,0);
        // for (auto &v : dp2) v.assign(mx+1,0);
        for (int j=0; j<l; j++) {
            dp2[0] |= dp1[j];
            if (j == 0) {
                dp2[l] |= (dp1[j] << tr[l][j]);
