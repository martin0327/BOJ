...
void solve() {
    ...
    vi a(n); ri(a);
    int n1 = n/2, n0 = n-n1;
    fenwick_tree<int> fen0(n0), fen1(n1);
    for (int i=0; i<n; i++) {
        if (i&1) fen1.add(i/2,a[i]);
        else fen0.add(i/2,a[i]);
    }
    while (q--) {
        int t; ri(t);
        if (t == 1) {
            ...
            if (l == r) {
                int i = l/2, ans;
                if (l&1) ans = abs(fen1.sum(i,i+1));
                else ans = abs(fen0.sum(i,i+1));
                po(ans);
            }
            else {
                int l0 = l/2, l1 = (l+1)/2;
                ...
                if (l&1) swap(l0,l1);
                if (r&1) swap(r0,r1);
                auto x = fen0.sum(l0,r0+1);
                auto y = fen1.sum(l1,r1+1);
                po(abs(x-y));
            }
        }
        else {
