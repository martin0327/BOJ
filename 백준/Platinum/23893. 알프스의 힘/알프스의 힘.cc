...
void solve() {
    ...
    auto f = [&](int y, int k, int p)  {
        auto mod = [](int a, int m) -> int {
            return (a % m + m) % m;
        };

        auto power = [&](int base, int exp, int p) -> int {
            int res = 1;
            base = mod(base, p);
            while (exp > 0) {
                if (exp % 2 == 1) res = (int)((i128)res * base % p);
                base = (int)((i128)base * base % p);
                exp /= 2;
            }
            return res;
        };

        auto sqrt_mod = [&](int n, int p) {
            vi ret;
            n = mod(n, p);
            if (n == 0) return ret = {0};
            if (p == 2) return ret = {n};
            if (power(n, (p - 1) / 2, p) != 1) return ret; 
        ...
        int y_sq = (int)((i128)y * y % p);
        int delta = mod(4 * k - 3 * y_sq, p);

        vi delta_roots = sqrt_mod(delta, p);
        
        if (delta_roots.empty()) return ret;

        vi solutions;
        int inv2 = power(2, p - 2, p); 

        for (int r : delta_roots) {
            int x = mod(mod(-y + r, p) * inv2, p);
            solutions.push_back(x);
        }

        if (solutions.size() == 2 && solutions[0] == solutions[1]) {
            solutions.pop_back();
        }
