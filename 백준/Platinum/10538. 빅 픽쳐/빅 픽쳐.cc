...
void solve() {
    i128 p = 31, m = 914217179575780481;
    ...
    vector<i128> c(n1);
    for (int i=0; i<n1; i++) {
        auto rk = rabin_karp(a[i],p,m);
        c[i] = rk.query(0,m1-1);
    }
    vector d(n2, vector<i128>(m2-m1+1));
    for (int i=0; i<n2; i++) {
        auto rk = rabin_karp(b[i],p,m);
        for (int j=0; j+m1-1<m2; j++) {
            d[i][j] = rk.query(j,j+m1-1);
        }
    }
    int ans = 0;
    int R = n2, C = m2-m1+1;

    auto z_function = [&] (vector<i128> &a) {
        int n = a.size();
        vi z(n);
        if (n == 0) return z;
