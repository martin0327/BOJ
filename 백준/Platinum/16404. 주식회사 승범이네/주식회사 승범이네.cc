...
void solve() {
    ...
    f(0,-1);
    lazy_segtree<S, op, e, F, mapping, composition, id> seg(2*n);
    fenwick_tree<int> fen(2*n);
    ...
    while (m--) {
        ...
        if (t==1) {
            ...
            int i; cin >> i;
            i--;
            int ans = fen.sum(0,out[i]);
            po(ans);
            // auto [x,y] = seg.get(in[i]);
            // cout << x << endl;
        }
