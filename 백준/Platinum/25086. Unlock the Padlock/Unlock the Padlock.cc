...
void solve() {
    ...
    function<int(int,int,int)> f= [&] (int l, int r, int j) {
        if (dp[l][r][j] != -1) return dp[l][r][j];
        int ret = inf;
        if (l == r) {
            return dp[l][r][j] = 0;
        }
        if (j == 0) {
            chmin(ret, f(l+1,r,0) + g(a[l],a[l+1]));
            chmin(ret, f(l+1,r,1) + g(a[l],a[r]));
        }
        else {
            chmin(ret, f(l,r-1,0) + g(a[r],a[l]));
            chmin(ret, f(l,r-1,1) + g(a[r],a[r-1])); 
        }
        return dp[l][r][j] = ret;
    };
    int ans = f(0,n-1,0) + g(0,a[0]);
    chmin(ans, f(0,n-1,1) + g(0,a[n-1]));
    po(ans);
