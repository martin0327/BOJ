...
struct HopcroftKarp {
  vi g, l, r;
  int ans;
  HopcroftKarp(int n, int m, const vp &e) : g(e.size()), l(n, -1), r(m, -1), ans(0) {
    vi deg(n + 1);
    ...
    while (true) {
      ...
      int t = 0;
      for (int i = 0; i < n; i++) if (l[i] == -1) q[t++] = a[i] = p[i] = i;
      bool match = false;
      for (int i = 0; i < t; i++) {
        int x = q[i];
        if (~l[a[x]]) continue;
...
void solve() {
  ...
  for (int j=0; j<m; j++) {
    bool ok = false;
    for (int i=0; i<n; i++) if (!a[i][j]) ok = true;
      ...
      for (int r=0; r<n; r++) {
        for (int c=0; c<m; c++) {
          ...
          for (int l=0; l<2; l++) {
            ...
            if (nc&1) e.emplace_back(mp[{r,c}], mp[{nr,nc}]);
            else e.emplace_back(mp[{nr,nc}], mp[{r,c}]);
          }
        }
      }

      HopcroftKarp hk(sz0,sz1,e);
      int cnt = 0;
      for (int r=0; r<n; r++) {
        for (int c=0; c<m; c++) {
          if (!a[r][c] && c!=j) cnt++;
