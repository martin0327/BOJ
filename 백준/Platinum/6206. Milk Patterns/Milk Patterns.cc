...
void solve() {
    ...
    using mint2 = dynamic_modint<1>;
    mint1::set_mod((int)1e9+7);
    mint2::set_mod(998244353);
    vector<mint1> ppow1(n+1,1), pinv1(n+1);
    vector<mint2> ppow2(n+1,1), pinv2(n+1);
    for (int i=1; i<=n; i++) {
        ppow1[i] = ppow1[i-1] * p1;
        ppow2[i] = ppow2[i-1] * p2;
    }
    pinv1[n] = ppow1[n].inv();
    pinv2[n] = ppow2[n].inv();
    for (int i=n-1; i>=0; i--) {
        pinv1[i] = pinv1[i+1] * p1;
        pinv2[i] = pinv2[i+1] * p2;
    }
    vector<mint1> h1(n+1);
    vector<mint2> h2(n+1);
    for (int i=0; i<n; i++) {
        h1[i+1] = h1[i] + (a[i]+1)*ppow1[i];
        h2[i+1] = h2[i] + (a[i]+1)*ppow2[i];
    }
    int ans = 0, lo = 1, hi = n;
    while (lo <= hi) {
        int mid = (lo + hi) / 2;
        ...
        for (int i=0; i+mid<=n; i++) {
            mint1 x = (h1[i+mid]-h1[i])*pinv1[i];
            mint2 y = (h2[i+mid]-h2[i])*pinv2[i];
            int xx = x.val();
            int yy = y.val();
            if (++mp[{xx,yy}] == k) ok = true;
        }
        if (ok) {
            ans = mid;
            lo = mid + 1;
