...
void solve() {
    ...
    function<vi(vi&)> f = [&](vi &a) {
        ...
        int m = 1;
        for (int l=0; l<n; l++) {
            vi b(ret.begin(), ret.begin()+m);
            int idx = 0;
            int i=0, j=0, k=0;
            while (i<m || j<m || k<m) {
                if (j==m && k==m) ret[idx++] = b[i++]-a[l];
                else if (i==m && k==m) ret[idx++] = b[j++];
                else if (i==m && j==m) ret[idx++] = b[k++]+a[l];
                else if (i==m) {
                    if (b[j] <= b[k]+a[l]) ret[idx++] = b[j++];
                    else ret[idx++] = b[k++]+a[l];
                }
                else if (j==m) {
                    if (b[i]-a[l] <= b[k]+a[l]) ret[idx++] = b[i++]-a[l];
                    else ret[idx++] = b[k++]+a[l];
                }
                else if (k==m) {
                    if (b[i]-a[l] <= b[j]) ret[idx++] = b[i++]-a[l];
                    else ret[idx++] = b[j++];
                }
                else {
                    if (b[i]-a[l]<=b[j] && b[i]-a[l]<=b[k]+a[l]) ret[idx++] = b[i++]-a[l];
                    else if (b[j]<=b[i]-a[l] && b[j]<=b[k]+a[l]) ret[idx++] = b[j++];
                    else ret[idx++] = b[k++]+a[l];
                }
            }
            m *= 3;
