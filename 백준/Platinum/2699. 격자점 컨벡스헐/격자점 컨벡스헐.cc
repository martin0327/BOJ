...
int ccw(pt p1, pt p2, pt p3) {
    return cross(diff(p2,p1), diff(p3,p1));
}

vector<pt> cvh(vector<pt>& a) {
    int n = a.size(), k = 0;
    if (n <= 2) return a;
    ...
    for (int i=0; i<n; i++) {
        while (k>=2 && ccw(hull[k-2], hull[k-1], a[i]) <= 0) k--;
        hull[k++] = a[i];
    }
    
    for (int i=n-2, t=k+1; i>=0; i--) {
        while (k>=t && ccw(hull[k-2], hull[k-1], a[i]) <= 0) k--;
        hull[k++] = a[i];
    }
...
vector<pt> rot_cal(const vector<pt>& hull) {
    int n = hull.size();
    if (n == 2) return hull;
    assert(n > 2);
    ...
    for (int i=0; i<n; i++) {
        ...
        while (abs(ccw(hull[i], hull[ni], hull[(j + 1) % n])) > 
               abs(ccw(hull[i], hull[ni], hull[j]))) {
            j = (j + 1) % n;
        }
        
        if (maxd < sqd(hull[i], hull[j])) {
            maxd = sqd(hull[i], hull[j]);
            ans = {hull[i], hull[j]};
        }
        if (maxd < sqd(hull[ni], hull[j])) {
            maxd = sqd(hull[ni], hull[j]);
            ans = {hull[ni], hull[j]};
        }
    }
...
void solve() {
    ...
    {
        auto t = h;
        sort(t.begin(), t.end(), [&] (pt a, pt b) {
            auto [x1,y1] = a;
            auto [x2,y2] = b;
            return make_pair(-y1,x1) < make_pair(-y2,x2);
        });
        init = {t[0].x, t[0].y};
    }
    reverse(h);
