...
void solve() {
    ...
    function<ld(int,int,int)> f = [&] (int r, int s, int p) {
        ld ret = 0;
        int m = r + s + p;
        if (m == 1) return ret = base;
        if (vis[r][s][p]) return dp[r][s][p];
        if (r > 0) chmax(ret, f(r-1,s,p) + (p*W+s*E)/(m-1));
        if (s > 0) chmax(ret, f(r,s-1,p) + (r*W+p*E)/(m-1));
        if (p > 0) chmax(ret, f(r,s,p-1) + (s*W+r*E)/(m-1));
        vis[r][s][p] = 1;
        return dp[r][s][p] = ret;
    };
    int r,s,p,mx = 0;
    ...
    while (r+s+p > 0) {
        ...
        else if (s > 0 && eq(f(r,s-1,p) + (r*W+p*E)/(m-1), f(r,s,p))) {
            ans += 'S';
            s--;
        }
        else {
