...
void solve() {
    ...
    for (int i=0; i<2*n; i++) {
        ...
        while (it2 != lr.end()) {
            ...
            int mid = (l+r)/2;
            if (mid < i) {
                if (i <= r) ms2.insert({2*r-l,l,r});
                it2++;
            }
            else break;
        }

        while (it3 != lr.end() && it3->second <= i) {
            chmax(mxL, it3->first);
            it3++;
        }
        ...
        if (ms1.size()) {
            while (ms1.size()) {
                auto [w,l,r] = *ms1.begin();
                int mid = (l+r)/2;
                if (l <= i && i <= mid) {
                    chmin(ans[idx], w+i);
                    break;
                }
                else ms1.erase(ms1.begin());
            }
        }
        if (ms2.size()) {
            while (ms2.size()) {
                auto [w,l,r] = *ms2.begin();
                int mid = (l+r)/2;
                if (mid < i && i <= r) {
                    chmin(ans[idx], w-i);
                    break;
                }
                else ms2.erase(ms2.begin());
            }
        }
        chmin(ans[idx], i-mxL);
        {
            auto it = lr.lower_bound({i,-inf});
            if (it != lr.end()) {
                chmin(ans[idx], it->second - i);
            }
        }
    }
    po(ans);
