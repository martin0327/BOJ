...
template<typename T>
pair<bool, vector<vector<T>>> mat_inv(vector<vector<T>> a) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (id == -1) return {false, vector<vector<T>>(1)};
...
void solve() {
    int A,B; ri(A,B);
    string temp = "cc cc2 vv vv2 ss ss2 cv sv vc sc vs cs";
    auto a = split_str(temp);
    int n = a.size();
    map<string,int> idx;
    for (int i=0; i<n; i++) {
        idx[a[i]] = i;
    ...
    for (int i=0; i<n; i++) {
        ...
        for (int j=0; j<n; j++) {
            auto t = a[j];
            if (s[1] != t[0]) continue;
            if (s.back() == '2' && t.back() == '2') continue;

            set<char> sc = {s[0],t[0],t[1]};
            if (s[0] != 'c' && sc.size() == 1) continue;

            if (t.back() == '2') b[i][j] = 1;
            else {
                if (t.back() == 'c') b[i][j] = 21;
                else if (t.back() == 'v') b[i][j] = 5;
                else if (t.back() == 's') b[i][j] = 3;
                if (t.front() == t.back()) b[i][j] -= 1;
            }
    ...
    auto f = [&] (int x) {
        ...
        if (x == 1) return mint(26);
        x--;
        vm init(n);
        init[9] = 21;
        init[7] = 5;

        auto [_,inv] = mat_inv(b);
        auto s = mat_id<mint>(n);
        for (int i=0; i<n; i++) {
            for (int j=0; j<n; j++) {
                s[i][j] -= b[i][j];
            }
        }
        s = mat_inv<mint>(s).second;
        auto r1 = mat_id<mint>(n);
        auto r2 = mat_exp(b,x+1);
        for (int i=0; i<n; i++) {
            for (int j=0; j<n; j++) {
                r1[i][j] -= r2[i][j];
            }
        ...
        mint ret = 0;
        for (int i=0; i<n; i++) {
            if (a[i][1] != 's') ret += res[i];
        }
        return ret;


        // auto mat = b^x;
        // auto mat = b.inv2().second;
        // for (int k=0; k<=x; k++) {
        //     auto mat = b^k;
