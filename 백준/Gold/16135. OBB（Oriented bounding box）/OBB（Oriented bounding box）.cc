...
void solve() {
    ...
    auto get_mid = [&] (pii a, pii b) {
        auto [x1,y1] = a;
        auto [x2,y2] = b;
        pii ret = {(x1+x2)/2, (y1+y2)/2};
        return ret;
    };
    auto am = get_mid(a[0],a[2]);
    auto bm = get_mid(b[0],b[2]);

    auto diff = [&] (pii a, pii b) {
        auto [x1,y1] = a; 
        auto [x2,y2] = b; 
        pii ret = {x1-x2, y2-y1};
        return ret;
    ...
    auto A1 = diff(get_mid(a[0],a[1]),am);
    auto A2 = diff(get_mid(a[1],a[2]),am);
    auto B1 = diff(get_mid(b[0],b[1]),bm);
    auto B2 = diff(get_mid(b[1],b[2]),bm);
    ...
    auto f = [&] (pii u) {
        ...
        return abs(dot(d,u)) >= res;
    };

    int ans = 1;
    vp us = {diff(a[0],a[1]), diff(a[1],a[2]), diff(b[0],b[1]), diff(b[1],b[2])};
    for (auto u : us) {
        if (f(u)) ans = 0;
    }
    po(ans);
