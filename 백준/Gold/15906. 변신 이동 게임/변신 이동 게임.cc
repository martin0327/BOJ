...
void solve() {
    ...
    auto get_nb = [&] (vi &a, int x) {
        auto it =  lower_bound(a.begin(), a.end(), x);
        vi ret;
        if (it != a.end()) {
            if (*it == x) {
                if (next(it) != a.end()) ret.push_back(*next(it));
            }
            else ret.push_back(*it);
        }
        if (it != a.begin()) ret.push_back(*prev(it));
        return ret;
    };
    Vec<3,int> dist(n,n,2,inf);
    min_pq<ti4> pq;
    dist[0][0][0] = 0;
    pq.push({0,0,0,0});
    auto update = [&] (int nd, int nr, int nc, int nj) {
        if (nd < dist[nr][nc][nj]) {
            dist[nr][nc][nj] = nd;
            pq.push({nd,nr,nc,nj});
        }
    ...
    while (pq.size()) {
        ...
        if (j == 0) {
            ...
            auto rnb = get_nb(rows[r],c);
            auto cnb = get_nb(cols[c],r);
            for (auto nc : rnb) {
                int nd = d + 1;
                int nr = r, nj = j;
                update(nd,nr,nc,nj);
