...
void solve() {
    ...
    auto f1 = [&] (int r, int c) {
        int ret = 0;
        if (r > 0  && c > 0) {
            for (int i=max(0ll,r-1); i<=r; i++) {
                for (int j=max(0ll,c-1); j<=c; j++) {
                    ret += a[i][j];
                }
            }
        }
    ...
    auto f3 = [&] (int r, int c, int d, int flag) {
        int ret = 0;
        vp b;
        b.push_back({r,c});
        b.push_back({r+dr[d],c+dc[d]});
        if (flag) d = (d+1) % 4; 
        else d = (d+3) % 4;
        b.push_back({r+dr[d], c+dc[d]});
        b.push_back({r+2*dr[d], c+2*dc[d]});
        bool ok = true;
        for (auto [nr,nc] : b) {
            if (check(nr,nc)) ret += a[nr][nc];
            else ok = false;
        }
        return ok?ret:0ll;
    };
    auto f4 = [&] (int r, int c, int d, int flag) {
        int ret = 0;
        vp b;
        for (int i=0; i<4; i++) {
            b.push_back({r,c});
    ...
    auto f5 = [&] (int r, int c, int d) {
        int ret = a[r][c];
        vp b;
        for (int i=0; i<4; i++) {
            if (i == d) continue;
            b.push_back({r+dr[i],c+dc[i]});
        }
        bool ok = true;
        for (auto [nr,nc] : b) {
            if (check(nr,nc)) ret += a[nr][nc];
    ...
    for (int i=0; i<n; i++) {
        for (int j=0; j<m; j++) {
            ...
            for (int d=0; d<4; d++) {
                chmax(ans, f5(i,j,d));
                for (int f=0; f<2; f++) {
                    chmax(ans, f3(i,j,d,f));
                    chmax(ans, f4(i,j,d,f));
                }
            }
        }
    }
