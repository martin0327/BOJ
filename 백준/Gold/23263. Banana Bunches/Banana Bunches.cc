...
void solve() {
    ...
    auto get_idx = [&] (int x) {
        ...
        if (it == comp.end() || *it != x) return ret = -1;
        else return ret = it - comp.begin();
    };
    ...
    for (int i=n; i>=2; i--) {
        for (int j=i; j<=n; j++) {
            ...
            if (idx != -1) {
                dp_suf[idx].push_back(min(dp_suf[idx].back(), sz));
            }
        }
    }
    ...
    for (int j=1; j<=n; j++) {
        {
            ...
            for (int r=l; r<=n; r++) {
                ...
                int sz = r-l+1;
                if (idx != -1) {
                    if (dp_suf[idx].size()) dp_suf[idx].pop_back();
                }
            }
        }
        for (int i=1; i<=j; i++) {
            ...
            if (s == k) {
                ...
                int tg = k - s;
                int idx = get_idx(tg);
                if (idx != -1 && dp_suf[idx].size()) {
                    chmin(ans, sz + dp_suf[idx].back());
                }
            }
        }
    }
