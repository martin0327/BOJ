...
void solve() {
    ...
    for (int i=1; i<=n; i++) {
        for (int j=1; j<=m; j++) {
            rpre[i][j] = rpre[i][j-1] + a[i][j];
            chmin(rpmin[i][j], min(rpmin[i][j-1], rpre[i][j]));
        }
        for (int j=m; j>=1; j--) {
            rsuf[i][j] = rsuf[i][j+1] + a[i][j];
            chmin(rsmin[i][j], min(rsmin[i][j+1], rsuf[i][j]));
        }
    }
    for (int j=1; j<=m; j++) {
        for (int i=1; i<=n; i++) {
            cpre[i][j] = cpre[i-1][j] + a[i][j];
            chmin(cpmin[i][j], min(cpmin[i-1][j], cpre[i][j]));
        }
        for (int i=n; i>=1; i--) {
            csuf[i][j] = csuf[i+1][j] + a[i][j];
            chmin(csmin[i][j], min(csmin[i+1][j], csuf[i][j]));
        }
    }
    ...
    for (int i=1; i<=n; i++) {
        for (int j=1; j<=m; j++) {
            chmax(ans, a[i][j]);
            int rmax = a[i][j];
            chmax(rmax, rsuf[i][j] - rsmin[i][j+1]);
            chmax(rmax, rpre[i][j] - rpmin[i][j-1]);
            // chmax(rmax, rsmin[i][1] - rsuf[i][j+1]);
            int cmax = a[i][j];
            chmax(cmax, csuf[i][j] - csmin[i+1][j]);
            chmax(cmax, cpre[i][j] - cpmin[i-1][j]);
            int t = rmax + cmax - a[i][j];
            // debug(j,t,rmax,cmax);
            chmax(ans, t);
        }
    }
