...
void solve() {
    ...
    Vec<3,int> dp(n+1,k1+1,k2+1,-inf);
    ...
    for (int i=0; i<n; i++) {
        ...
        for (int j=0; j<=k1; j++) {
            for (int l=0; l<=k2; l++) {
                if (i-j < sz1 && i-l < sz2) {
                    chmax(dp[ni][j][l], dp[i][j][l] + a[i-j]*b[i-l]);
                }
                if (j < k1) chmax(dp[ni][j+1][l], dp[i][j][l]);
                if (l < k2) chmax(dp[ni][j][l+1], dp[i][j][l]);
                if (j < k1 && l < k2) chmax(dp[ni][j+1][l+1], dp[i][j][l]);
            }
        }
    }
    int ans = dp[n][k1][k2];
