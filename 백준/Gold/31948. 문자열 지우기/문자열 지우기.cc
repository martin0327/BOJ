...
void solve() {
    ...
    for (int i=0; i<3; i++) {
        const auto &t = b[i];
        for (int j=0; j<n; j++) {
            if (t[j] != '?') a[i][t[j] - '0'].push_back(j);
        }
    }
    Vec<3,int> dp(n,n,3,inf);
    function<int(int,int,int)> f = [&] (int l, int r, int q) {
        int ret = 0;
        if (l > r) return ret = 0;
        if (dp[l][r][q] != inf) return dp[l][r][q];
        if (q == 2 && l <= idx && idx <= r) {
            ret |= (f(l,r,0) == 0);
            ret |= (f(l,r,1) == 0);
        }
        ...
        if (t[l] != '?') { // left
            int x = (t[l] - '0') ^ 1;
            auto it = lower_bound(a[q][x].begin(), a[q][x].end(), l);
            // debug(x);
            // debug(a[q][x]);
            // debug(it != a[q][x].end());
            int lim = (it != a[q][x].end()) ? (*it) : n;
            if (q == 2 && idx != -1) {
                chmin(lim, idx);
            }
            // debug(lim);
            for (int j=l; j<min(r+1,lim); j++) {
                ret |= (f(j+1,r,q) == 0);
            }
        }
        if (t[r] != '?') {
            int x = (t[r] - '0') ^ 1;
            auto it = lower_bound(a[q][x].begin(), a[q][x].end(), r);
            // debug(it != a[q][x].begin());
            int lim = (it != a[q][x].begin()) ? *prev(it) : -1;
            if (q == 2 && idx != -1) {
                chmax(lim, idx);
            }
            // debug(lim);
            for (int j=r; j>max(l-1,lim); j--) {
                ret |= (f(l,j-1,q) == 0);
            }
        }
        return dp[l][r][q] = ret;
    ...
    // debug(dp);
    po(ans);
    // win = 1, lose = 0; if there is at least one choice that forces opponent to lose, then i win
