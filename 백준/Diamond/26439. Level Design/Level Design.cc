...
void solve() {
    ...
    for (int i=0; i<m; i++) {
        auto [sz,cnt] = b[i];
        dp[i+1] = dp[i];
        vector<deque<int>> vdq(sz);
        for (int j=0; j<=n; j++) {
            int q = j / sz;
            int r = j % sz;
            auto &dq = vdq[r];
            if (dq.size() && (q - dq.front()/sz > cnt)) {
                dq.pop_front();
            }
            while (dq.size()) {
                int k = dq.back();
                if ((dp[i][k]+k/sz)>=(dp[i][j]+j/sz)) dq.pop_back();
                else break;
            }
            dq.push_back(j);
            chmin(dp[i+1][j],dp[i][dq.front()] + (j-dq.front())/sz);
        }
    }
    vi ans = dp[m];
    reverse(b);
