...
void solve() {
    ...
    auto get_idx = [&] (vi &cx, int x) {
        return lower_bound(cx.begin(), cx.end(), x) - cx.begin();
    };
    auto get_ridx = [&] (int x) {
        auto it = upper_bound(cx.begin(), cx.end(), x+A);
        it--;
        return it - cx.begin();
    };
    auto get_lidx = [&] (int x) {
        auto it = lower_bound(cx.begin(), cx.end(), x-A);
        return it - cx.begin();
    };
    ...
    for (int i=0; i<n; i++) {
        auto [x,y] = a[i];
        y2xi[y].push_back({x,i});
    }

    {
        segtree<S,op,e> seg(xsz);
        vvi rmv(ysz);
        vector<queue<pii>> q(xsz);
        for (int yi=ysz-1; yi>=0; yi--) {
            for (auto xj : rmv[yi]) {
                assert(!q[xj].empty());
                q[xj].pop();
                if (q[xj].size()) {
                ...
                else seg.set(xj,neg_inf);
            }
            for (auto [xi,i] : y2xi[yi]) {
                int x = cx[xi];
                int y = cy[yi];
                {
                    auto ridx = get_ridx(x);
                    auto lidx = xi;
                    auto [ty,j] = seg.prod(lidx,ridx+1);
                    if (ty != -inf) {
                        adj[i].push_back(j);
                    }
                }
                {
                    auto lidx = get_lidx(x);
                    auto ridx = xi;
                    auto [ty,j] = seg.prod(lidx,ridx+1);
                    if (ty != -inf) {
                        adj[i].push_back(j);
                ...
                }
                {
                    ...
                    q[xi].push(tg);
                    // greast ny s.t. ny < y-B
                    auto idx = get_idx(cy, y-B);
                    if (idx > 0) rmv[idx-1].push_back(xi);
                }
            }
        }
    }
    ...
    for (int i=0; i<n; i++) adj[i] = get_unique(adj[i]);
    vi vis(n);
    queue<int> q;
    for (int i=0; i<n; i++) {
        auto [xi,yi] = a[i];
