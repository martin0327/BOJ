...
void solve() {
    ...
    function<void(int)> f2 = [&] (int u) {
        for (int j=2; j<m; j++) {
            if (sp[u][j-1] != -1) sp[u][j] = sp[sp[u][j-1]][j-1];
            else break;
        }
        for (auto v : adj[u]) {
            f2(v);
    ...
    function<void(int)> f3 = [&] (int i) {
        for (int j=2; j<m; j++) {
            if (sp[i][j] != -1) {
                int u = sp[i][j];
                int v = sp[i][j-1];
                aa[i][j] = aa[v][j-1] * aa[i][j-1] + (1-aa[v][j-1]) * bb[i][j-1];
                bb[i][j] = bb[v][j-1] * aa[i][j-1] + (1-bb[v][j-1]) * bb[i][j-1];
            }
            else break;
        }
        for (auto v : adj[i]) {
    ...
    // function<void(int,int,int)> g = 
    // [&](int root, int u, int p) {
    //     if (root != u) {
    //         aa[root][u] = a[u]*aa[root][p] + b[u]*(1-aa[root][p]);
    //         bb[root][u] = a[u]*bb[root][p] + b[u]*(1-bb[root][p]);
    //     }
    //     else {
    //         aa[root][u] = 1;
    //         bb[root][u] = 0;
    ...
    while (q--) {
        ...
        vector<pair<mint,mint>> temp;
        {
            ...
            while (d > 0) {
                ...
                au.push_back({cur,idx+1});
                cur = sp[cur][idx+1];
            }
            mint aaa = 1;
            mint bbb = 0;
            for (auto [u,d] : au) {
                mint naaa = aa[u][d] * aaa + (1-aa[u][d]) * bbb;
                mint nbbb = bb[u][d] * aaa + (1-bb[u][d]) * bbb;
                aaa = naaa;
                bbb = nbbb;
            }
            temp.push_back({aaa,bbb});
            // debug(cur,anc);
            // assert(cur == anc);
            // debug(au);
        }
        ...
        res += pr[anc] * (temp[0].first * temp[1].first);
        res += pc[anc] * (temp[0].second * temp[1].second);
        // debug(anc);
        // debug(pr[anc], pc[anc]);
        // res = pr[anc] * (aa[anc][u] * aa[anc][v]);
        // res += pc[anc] * (bb[anc][u] * bb[anc][v]);
        ans.push_back(res);
    }
    po(ans);
    // debug(k*5);
    // debug(aa[0][3]*10/8,bb[0][3]*10);
    // debug(aa[0][4]*100/24);
    // debug(bb[0][4]*)
