...
void solve() {
    ...
    //         t.insert(x);
    //     }
    //     a[i] = vi(t.begin(), t.end());
    // }
    // vi L(m), U(m);
    // for (int i=0; i<m; i++) {
    //     L[i] = rng() % (n/15);
    //     L[i]++;
    //     int delta = rng() % (n-L[i]+1);
    //     U[i] = L[i] + delta;
    //     assert(L[i] <= U[i]);
    // }
    // debug(a);
    ...
    for (int lim=1; lim<=m; lim++) {
        int sz = n+m+2;
        int src = sz-2, tg = sz-1;
        mf_graph<int> g(sz);
        for (int i=0; i<n; i++) {
            g.add_edge(src,i,lim);
        }
        for (int i=0; i<m; i++) {
            g.add_edge(n+i,tg,L[i]);
        }
        for (int i=0; i<n; i++) {
            for (auto j : a[i]) {
                g.add_edge(i,n+j,1);
