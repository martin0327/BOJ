...
using S = int;
S op (int x, int y) { return max(x,y); }
S e() { return 0ll; }
...
void solve() {
    ...
    auto dbg_dq = [&] (vector<deque<int>> a) {
        for (auto &b : a) {
            vi dq(b.begin(), b.end());
            debug(dq);
        }
    ...
    auto push = [&] (int i, bool back) {
        auto &q = dq[b[i]];
        if (back) q.push_back(i);
        else q.push_front(i);
        // seg.set(b[i], q.back() - q.front());
        seg.set(b[i], q.size() ? q.back() - q.front() : 0ll);
    };

    auto pop = [&] (int i, bool back) {
        auto &q = dq[b[i]];
        if (back) q.pop_back();
        else q.pop_front();
        seg.set(b[i], q.size() ? q.back() - q.front() : 0ll);
