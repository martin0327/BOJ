...
struct EulerTour
{
    ...
    pair<int, int> subtree(int a) { return ETT[a]; }
    vector<pair<int, int>> rootpath(int a) { return rngs[a]; }
...
using S = pair<ull,ull>;
using F = pair<ull,ull>;

S op(S a, S b) {
    auto [x,u] = a;
    auto [y,v] = b;
    return {(x+y)%mod,(u+v)%mod};
}
...
S mapping(F f, S s) {
    auto [a,b] = f;
    auto [x,y] = s;
    return {((a*x)%mod + (b*y)%mod)%mod, y};
}
 
F composition(F f, F g) {
    auto [a,b] = g;
    auto [c,d] = f;
    return {(c*a)%mod, ((c*b)%mod+d)%mod};
}
...
void solve() {
    ...
    while (q--) {
        ...
        if (t==1) {
            ...
            seg.apply(l,r,make_pair(1,w));
        }
        else if (t==2) {
            int u,v,w;
            ri(u,v,w);
            u--; v--;
            for (auto [l,r] : et.path(u,v)) {
                seg.apply(l,r, make_pair(1,w));
            }
        }
        else if (t==3) {
