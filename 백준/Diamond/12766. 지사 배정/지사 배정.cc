...
void solve() {
    ...
    vi a;
    for (int i=0; i<m; i++) a.push_back(dist1[i]+dist2[i]);
    dsort(a);
    ...
    for (int j=2; j<=m; j++) {
        ...
        function<void(int,int,int,int)> f = [&] (int l, int r, int s, int e) {
            if (l > r) {
                return;
            }
            int mid = (l+r)/2;
