...
using i32 = signed;
const i32 inf = 1e9;
const i32 mx_sz = 2e5+1;
const i32 mx_dist = 1e6+1;
vp adj[mx_sz];
i32 sub[mx_sz], check[mx_sz];
i32 d2c[mx_dist];
// unordered_map<i32,i32> mp;
// unordered_map<i32,i32> mp2;
vi b,B;
...
void solve() {
    ...
    i32 ans = inf;
    function<i32(i32,i32)> get_sub = [&] (i32 u, i32 p) {
        sub[u] = 1;
        for (auto &[v,w] : adj[u]) {
            if (v == p) continue;
            if (check[v]) continue;
    ...
    function<i32(i32,i32,i32)> get_cen = [&] (i32 u, i32 p, i32 cap) {
        i32 ret = u;
        for (auto &[v,w] : adj[u]) {
            if (v == p) continue;
            if (check[v]) continue;
            if (2*sub[v] > cap) ret = get_cen(v,u,cap);
        }
        return ret;
    };
    ...
    function<void(i32,i32)> cen_dec = [&] (i32 u, i32 p) {
        int cap = get_sub(u,p);
        auto cen = get_cen(u,p,cap);
        vi B;
        for (auto &[v,w] : adj[cen]) {
            if (v == p) continue;
            if (check[v]) continue;
            vp b;
            function<void(i32,i32,i32,i32)> g = [&] (i32 u, i32 p, i32 cum, i32 cnt) {
                if (cum > tg) return;
                b.push_back({cum,cnt});
                // chmin(d2c[cum],cnt);
                if (cum == tg) {
