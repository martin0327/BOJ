...
void solve() {
    ...
    vi pre(n);
    function<void(int,int,int)> f0 = [&] (int u, int x, int p) {
        pre[u] = ((p>=0) ? pre[p] : 0) + x;
        for (auto [v,w] : adj[u]) {
            if (v==p) continue;
            f0(v,w,u);
    ...
    while (q--) {
        ...
        ri(b,c);
        vi d = b;
        for (auto u : c) d.push_back(u);
        sort(d.begin(), d.end(), [&](int x, int y) {return lca.idx[x] < lca.idx[y];});
        vi e;
        for (int i=0; i<d.size(); i++) {
            e.push_back(d[i]);
            if (i > 0) e.push_back(lca.query(d[i],d[i-1]));
        }
        e = get_unique(e);
        sort(e.begin(), e.end(), [&](int x, int y){return lca.idx[x] < lca.idx[y];});
        int sz = e.size();
        ...
        for (int i=1; i<sz; i++) {
            ...
            // u = comp[u];
            // p = comp[p];
            cadj[p].emplace_back(u,w);
            cadj[u].emplace_back(p,w);
        }
        for (auto i : e) {
            cadj[i] = get_unique(cadj[i]);
        }
        ...
        function<int(int,int,int,vi &,vi&)> g = [&] (int u, int p, int push, vi &b, vi &d) {
            int res = inf;
            if (b[u]) {
                res = 0;
                push = 0; 
            ...
            for (auto [v,w] : cadj[u]) {
                if (v==p) continue;
                chmin(res,w+g(v,u,min(inf,push+w),b,d));
            }
            chmin(d[u],res);
        ...
        g(root,-1,inf,b2,d2);
        int ans = inf;
        for (auto u : e) chmin(ans, d1[u]+d2[u]);
        po(ans);
