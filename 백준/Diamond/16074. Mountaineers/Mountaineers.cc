...
struct dsu {
  public:
    dsu() : _n(0) {}
    dsu(int n) : _n(n), parent_or_size(n, -1), a(n) {}

    int merge(int a, int b, int sz_a, int sz_b) {
        assert(0 <= a && a < _n);
        assert(0 <= b && b < _n);
        int x = leader(a), y = leader(b);
        if (x == y) return x;
        if (sz_a < sz_b) swap(x,y);
...
void solve() {
    ...
    int sz = n*m;
    vector<set<int>> qr(sz);
    debug(sz);

    auto c2i = [&] (int r, int c) {
        return m*r + c;
    };
    auto i2c = [&] (int u) {
        int r = u / m;
    ...
    for (auto [w,u,v] : e) {
        ...
        int lv = d.leader(v);
        if (!d.same(lu,lv)) {
            d.merge(lu,lv,qr[lu].size(), qr[lv].size());
            if (d.leader(lu) == lu) {
                u = lu;
                v = lv;
            }
