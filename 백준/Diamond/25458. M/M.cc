...
struct OnlineBCC {
    ...
    OnlineBCC(int n) : n(n), cc_dsu(n), bcc_dsu(n), par(n, -1), visited(n, 0), timer(0) {}

    void make_root(int u) {
        u = bcc_dsu.find(u);
        int curr = u, prev = -1;
        while (curr != -1) {
            int p = (par[curr] == -1) ? -1 : bcc_dsu.find(par[curr]);
            par[curr] = prev; 
            prev = curr;
            curr = p;
        }
    ...
    void add_edge(int u, int v) {
        ...
        if (cc_dsu.find(u) != cc_dsu.find(v)) {
            cc_dsu.unite(u, v);
            make_root(u);
            par[u] = v;
        } else {
            ...
            while (curr_u != -1 || curr_v != -1) {
                if (curr_u != -1) {
                    curr_u = bcc_dsu.find(curr_u);
                    if (visited[curr_u] == timer) { lca = curr_u; break; }
                    visited[curr_u] = timer;
                    curr_u = (par[curr_u] == -1) ? -1 : bcc_dsu.find(par[curr_u]);
                }
                if (curr_v != -1) {
                    curr_v = bcc_dsu.find(curr_v);
                    if (visited[curr_v] == timer) { lca = curr_v; break; }
                    visited[curr_v] = timer;
                    curr_v = (par[curr_v] == -1) ? -1 : bcc_dsu.find(par[curr_v]);
                }
            }

            auto condense_path = [&](int x) {
                x = bcc_dsu.find(x);
                while (x != lca) {
                    int p = (par[x] == -1) ? -1 : bcc_dsu.find(par[x]);
                    bcc_dsu.unite(x, lca); 
                    x = p;
                }
            };
    ...
    bool same_bcc(int u, int v) {
        return bcc_dsu.find(u) == bcc_dsu.find(v);
    }
};
