...
void solve() {
    ...
    //     for (int i=1; i*i<=x; i++) {
    //         if (x % i == 0) {
    //             if (check[i]) chmax(mx, i);
    //             check[i] = 1;
    //             int j = x / i;
    //             if (i != j) {
    //                 if (check[j]) chmax(mx,j);
    //             }
    //             check[j] = 1;
    //         }
    //     }
    ...
    if (k == 0) {
        ...
        int ans = max(max_d-1, max(a)-min_d);
        for (int i=sz; i>=sz-10; i--) {
            int l = sz, r = 0;
            for (auto x : a) {
                chmin(l,gcd(x,i));
                chmax(r,gcd(x,i));
            }
            chmax(ans, max(max_d,r) - min(min_d,l));
        }
        po(ans);
    }
    else if (k == 2) {
        int ans = max(max(a) - 1,sz-min_d);
        for (int i=sz; i>=sz-10; i--) {
            int l = sz, r = 0;
            for (auto x : a) {
                chmax(r,gcd(x,i));
            }
            chmax(ans, max(max_d,r) - 1);
        }
        for (int i=sz; i>=sz-10; i--) {
            int l = sz;
            for (auto x : a) {
            ...
            chmax(ans, max(max_d,i) - min(min_d,l));
        }
        po(ans);
    }
    else {
