...
S op(S l, S r) {
    int res = max({l.res,r.res,l.suf+r.pre});
    int pre = max(l.pre,l.sum+r.pre);
    int suf = max(r.suf,r.sum+l.suf);
    int sum = l.sum+r.sum;
    return {res,pre,suf,sum};
}
S e() {return {-inf,-inf,-inf,0ll};}
...
void solve() {
    ...
    int xsz, ysz;
    {
        vi t = get_unique(x);
        xsz = t.size();
        for (int i=0; i<xsz; i++) xinv[t[i]] = i;
    }
    {
        vi t = get_unique(y);
        ysz = t.size();
        for (int i=0; i<ysz; i++) yinv[t[i]] = i;
    }
    for (auto &t : x) t = xinv[t];
    for (auto &t : y) t = yinv[t];
    vvp a(xsz);
    for (int i=0; i<n; i++) {
    ...
    int ans = -inf;
    for (int i=0; i<xsz; i++) {
        segtree<S,op,e> seg(ysz);
        for (int j=0; j<ysz; j++) seg.set(j,{0,0,0,0});
        for (int j=i; j<xsz; j++) {
            for (auto [y,w] : a[j]) {
                int p = seg.get(y).res;
                seg.set(y,{p+w,p+w,p+w,p+w});
            }
            chmax(ans, seg.all_prod().res);
        }
    }
    po(ans);
}