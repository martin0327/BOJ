...
void solve() {
    int n; ri(n);
    vi a(n), par(n,-1), sub(n), check(n), dep(n);
    ri(a);
    vector<min_pq<pii>> pq(n);
    ...
    function<int(int,int)> gsub = [&] (int u, int p) {
        sub[u] = 1;
        for (auto v : adj[u]) {
            if (v==p) continue;
            if (check[v]) continue;
    ...
    function<int(int,int,int)> gcen = [&] (int u, int p, int cap) {
        int ret = u;
        for (auto v : adj[u]) {
            if (v == p) continue;
            if (check[v]) continue;
            if (2*sub[v] > cap) ret = gcen(v,u,cap);
        }
        return ret;
    };
