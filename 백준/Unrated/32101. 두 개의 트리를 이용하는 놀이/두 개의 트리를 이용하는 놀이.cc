...
void solve() {
    ...
    auto f = [&] (int n, vi b, vp edges) {
        dsu d(n);
        vp e;
        for (auto [u,v] : edges) {
            if (!b[u] && !b[v]) d.merge(u,v);
            else if (b[u] != b[v]) {
                if (b[u]) swap(u,v);
                e.push_back({u,v});
            }
        ...
        return make_pair(s,ws);
    };

    auto [s1,ws1] = f(n1,b1,e1);
    auto [s2,ws2] = f(n2,b2,e2);

    mint ans = s2*ws1 + s1*ws2;
    po(ans);
