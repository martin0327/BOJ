...
S op(S x, S y) {
    S ret;
    merge(x.begin(), x.end(), y.begin(), y.end(), back_inserter(ret));
    return ret;
}

S e() { S ret; return ret; }

template <class S, S (*op)(S, S), S (*e)()> struct mstree {
  public:
    mstree() : mstree(0) {}
    explicit mstree(int n) : mstree(std::vector<S>(n, e())) {}
    explicit mstree(const std::vector<S>& v) : _n(signed(v.size())) {
        log = internal::ceil_pow2(_n);
        size = 1 << log;
        d = std::vector<S>(2 * size, e());
        for (int i = 0; i < _n; i++) d[size + i] = v[i];
    ...
    int prod(int l, int r, int lo, int hi) {
        assert(0 <= l && l <= r && r <= _n);
        int ret = 0;
        l += size;
        r += size;
        ...
        auto f = [&] (S &a) {
            int ret = cnt_leq_x(a, hi) - cnt_lt_x(a,lo);
            return ret;
        };
...
mstree<S,op,e> tree[mxd+1];
vi check(mxd+1);
vvp b(mxd+1);

vi dd(mxn+1), ss(mxn+1);
...
signed main() {
    ...
    for (int d=1; d<=mxd; d++) {
        if (b[d].size() < 700) continue;
        check[d] = 1;
        vector<S> a(mxn+1);
        for (auto [y,s] : b[d]) {
            a[y].push_back(s);
        }
        tree[d] = mstree<S,op,e>(a);
    }
...
void solve() {
    ...
    for (int i=0; i<m; i++) {
        ...
        int l1 = max(1ll, x-A);
        int r1 = min(n, x+A);
        int l2 = max(1ll, dd[x]-B);
        int r2 = min(mxd,dd[x] + B);
        int l3 = max(1ll, ss[x]-C);
        int r3 = ss[x] + C;
        for (int d=l2; d<=r2; d++) {
            if (check[d]) {
                if (l1 <= r1 && l3 <= r3) {
                    ans[i] += tree[d].prod(l1,r1+1,l3,r3);
                }
            }
            else {
                for (auto [y,s] : b[d]) {
